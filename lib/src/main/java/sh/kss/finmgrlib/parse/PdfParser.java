...
public class PdfParser {
    ...
    private static final List<LineParser> LINE_PARSERS = ImmutableList.of(new QuestradePdfOld(), new QuestradePdf());
    ...
    public static List<InvestmentTransaction> parsePdf(File file) {
        ...
        try (PDDocument document = PDDocument.load(file)) {
            ...
            if (!document.isEncrypted()) {
                ...
                for (LineParser lineParser : LINE_PARSERS) {

                    // If it matches then short-circuit and return the parse results
                    if (lineParser.isMatch(lines)) {

                        LOG.debug(String.format("Matched parser %s", lineParser.toString()));

                        return lineParser.parse(lines);

                    } else {

                        LOG.debug(String.format("Did not match parser %s", lineParser.toString()));
                    }
                }
            } else {
